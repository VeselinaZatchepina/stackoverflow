...
fun ICorChainDsl<SContext>.stubUpdateSuccess(title: String) = worker {
    this.title = title
    ...
    handle {
        ...
        val stub = Question.getModel().apply {
            ...
            questionRequest.rating.takeIf { it.isNotBlank() }?.also { this.rating = it }
            questionRequest.ownerId.takeIf { it.isNotBlank() }?.also { this.ownerId = it }
            questionRequest.creationDate.also { this.creationDate = it }
